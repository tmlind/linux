				...
				struct gsm_serdev_dlci *ops)
{
	...
	/* Wait for gsm_dlci_close() to complete */
	for (retries = 10; retries > 0; retries--) {
		if (dlci->state == DLCI_CLOSED)
			return;
		msleep(100);
	}
	pr_err("n_gsm: dlci%i close timed out\n", dlci->addr);
}
			   ...
			   size_t count)
{
	...
	if (debug & 4)
		gsm_hex_dump_bytes(__func__, data, count);
...
int gsm_serdev_register_tty_port(struct gsm_serdev *gsd, int line)
{
	...
	dev = tty_register_device(gsm_tty_driver, base + ops->line, NULL);
	if (IS_ERR(dev)) {
		gsm_serdev_unregister_dlci(gsd, ops);
		kfree(ops);
		return PTR_ERR(dev);
	}
...
int gsm_serdev_register_device(struct gsm_serdev *gsd)
{
	...
	memcpy(&c, gsd->cfg, sizeof(c));
	error = gsm_config(gsm, &c);
	if (error) {
		gsm_cleanup_mux(gsd->gsm, false);
		/* Paired with kref_init() in gsm_alloc_mux() */
		mux_put(gsd->gsm);
		kfree(gsm);
		gsd->gsm = NULL;
